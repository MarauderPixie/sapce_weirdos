...
ui <- fluidPage(
    ...
    fluidRow(
        ...
        actionButton("print", "Print", onclick = "$('#unit_area').print();")
    ),
    
    fluidRow(
        column(4,
               h3("Choose A Warband Trait")
        ),
    ...
    fluidRow(
        column(8,
               radioGroupButtons(
                   "warband", label = "", selected = character(0),
...
server <- function(input, output) {
    ...
    output$warband_trait <- renderText({input$warband})
    output$trait_description <- renderText({pointlist$Description[input$warband == pointlist$Name]})
    ...
    output$eq_text <- renderText({pointlist$Description[input$eq == pointlist$Name]})
    ...
    observeEvent(input$add_unit, {
        unit <- list(
            ...
            desc_equip = pointlist$Description[pointlist$Name == input$eq],
            points = sum(c(
                pointlist$Points[pointlist$Name == input$speed & pointlist$Area == "Speed"],
                pointlist$Points[pointlist$Name == input$def & pointlist$Area == "Defense"],
                pointlist$Points[pointlist$Name == input$fp & pointlist$Area == "Firepower"],
            ...
            ui = column(4, tags$div(
                withTags({
                    table(
                        ...
                        tr(
                            td(unit$equip, colspan = 2), 
                            td(unit$desc_equip, colspan = 3)
                        ),
                        # if (!is.null(unit$psych)) {
                        for (i in unit$psych) {
                            tr(
                                td(i, colspan = 2),
                                td("another row", colspan = 3)
                            )
                        }
                    )
                })
